...
//在布局中使用的时候要调用的方法
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			if (currentLast != null) {
				//当执行到最后一个点的时候，整个view的透明度发生变化，变为原来的一半，因为画图的时候，画笔设置的透明度是设置的整个画布的透明度
				//也就是整个view的透明度
			...
			// 只刷新扫描框的内容，其他地方不刷新，这样的话这个界面会一直处于刷新的状态，其实就是一个死循环
			postInvalidateDelayed(ANIMATION_DELAY, frame.left, frame.top,
					frame.right, frame.bottom);

		}
		//Log.i("ssss","onDraw2::"+"正在运行");
	}

	public void drawViewfinder() {
		resultBitmap = null;
		invalidate();
//		Log.i("ssssss","drawViewfinder"+"zhengzaiyunxing ");
	}
